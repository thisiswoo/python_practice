...
total_time = 60 # 총 시간(초)

start_ticks = pygame.time.get_ticks() # 현재 틱을 받아옴(현재 시간)
...
while running:
    ...
    # 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # ms -> s 단위 변환
    display_time(total_time - int(elapsed_time)) # 시간 푝시
